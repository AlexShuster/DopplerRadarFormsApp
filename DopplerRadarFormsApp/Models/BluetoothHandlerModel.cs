    ...
    public class BluetoothHandlerModel
    {
        ...
        public Pitch Read()
        {
            ...
            try
            {
                ...
                if (bytesString.LastIndexOf(';') == 12 || bytesString.Last() == '\0') // aaaaa,bbbbbb;
                {
                    // Regex Splitters
            ...
            {
                double speedDouble;
                double.TryParse(speed, out speedDouble);
                double spinDouble;
                double.TryParse(spinRate, out spinDouble);
                try
                {
                    speedFinal = Convert.ToInt32(speedDouble);
                    spinRateFinal = Convert.ToInt32(spinDouble);
